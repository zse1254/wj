...
async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (!biliRes.ok) {
      ...
      if (data.ugcSeason) {
        htmlSeries = { season_id: data.ugcSeason.id, title: data.ugcSeason.title || "", videos: (data.ugcSeason.episodes || []).map((ep: any) => ({ bvid: ep.bvid, title: ep.title || "", cover_url: ep.cover || "", page: ep.page || 1, duration: ep.duration || 0 })) };
      }
